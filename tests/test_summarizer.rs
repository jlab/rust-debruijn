...
/* fn assert_valid<SD: SummaryData<DI>, DI>(valid: bool, data: &SD, expected_valid: bool, config: &SummaryConfig) {
    assert_eq!(valid, data.valid(config));
    assert_eq!(valid, expected_valid);
}

fn assert_p_fc<SD: SummaryData<DI>, DI>(data: &SD, expected_p: f32, expected_fc: f32, config: &SummaryConfig) {
    assert_eq!(expected_p, data.p_value(config).unwrap());
    assert_eq!(expected_fc, data.fold_change(config).unwrap());
}

fn assert_prints<SD: SummaryData<DI>, DI>(
    data: SD, 
    config: &SummaryConfig, 
    translator: &Translator,
    expected_print_dot: String,
    expected_print_ol: String,
    expected_print_json: String)
{
    assert_eq!(expected_print_dot, data.print(translator, config, None));
    assert_eq!(expected_print_ol, data.print_ol(translator, config, None));
    assert_eq!(expected_print_json, data.print_json(translator, config, None));
}

fn assert_mem<SD: SummaryData<DI>, DI>(data: &SD, expected_mem: usize) {
    assert_eq!(data.mem(), expected_mem);
}

fn assert_tags<SD: SummaryData<DI>, DI>(data: &SD, expected_tags: Tags) {
    assert_eq!(expected_tags, data.tags().unwrap());
}

fn assert_ids<SD: SummaryData<DI>, DI>(data: &SD, expected_ids: &[ID]) {
    assert_eq!(expected_ids:  ids.clone(), data.ids().unwrap());
}

fn assert_sum<SD: SummaryData<DI>, DI>(data: &SD, expected_sum: usize) {
    assert_eq!(expected_sum, data.sum().unwrap());
}

fn assert_quality<SD: SummaryData<DI>, DI>(data: &SD, expected_quality: BaseQuality) {
    assert_eq!(expected_quality, data.quality().unwrap());
}

fn assert_edge_mults<SD: SummaryData<DI>, DI>(data: &SD, expected_edge_mults: EdgeMult) {
    assert_eq!(expected_edge_mults:  edge_mults.clone(), *(data.edge_mults().unwrap()));
}

fn assert_fix_edge_mults<SD: SummaryData<DI>, DI>(data: &SD, exts: Exts, expected_fxed_edge_mults: EdgeMult) {
    let mut data = data.clone();
    data.fix_edge_mults(exts);
    assert_eq!(expected_fxed_edge_mults:  edge_mults.clone(), *(data.edge_mults().unwrap()));
}

fn assert_mapped_ids<SD: SummaryData<DI>, DI>(data: &SD, expected_mapped_ids: &[ID]) {
    assert_eq!(expected_mapped_ids:  ids.clone(), data.mapped_ids().unwrap());
}

fn assert_set_mapped_ids<SD: SummaryData<DI>, DI>(data: &SD, expected_mapped_ids: &[ID]) {
    assert_eq!(expected_mapped_ids:  ids.clone(), data.mapped_ids().unwrap());
}

fn assert_sample_count<SD: SummaryData<DI>, DI>(data: &SD, expected_sample_count: usize) {
    assert_eq!(expected_sample_count, data.sample_count().unwrap());
}

fn assert_summarizer<SD: SummaryData<DI>, DI>(expected_summarizer: Summarizers) {
    assert_eq!(expected_summarizer, SD::summarizer());
} */

// TODO: edge mults, fix edge mults, mapped IDs, set mapped ids:  ids.clone(), sample count, summarizer
...
fn test_summary_data() {
    ...
    let compare_data = SummaryTest {
        ...
        tags,
        mem: size_aligned(t_m + c_m_s + s_m, c_m_h, 8),
        sum,
        ids: None,
        p_value,
        fold_change,
    ...
    let compare_data = SummaryTest {
        ...
        print_json: "\"sum\": 6, \"samples\": [\"0\", \"1\", \"2\", \"3\", \"7\", \"8\"], \"counts\": [1, 1, 1, 1, 1, 1], \"p_value\": 0.39023498, \"fold_change\": 5.4498405".to_string(),
        tags,
        mem: size_aligned(t_m + c_m_s, c_m_h, 8),
        sum,
        ids: None,
        p_value,
        fold_change,
    ...
    let compare_data = SummaryTest {
        ...
        print_json: "\"sum\": 6, \"samples\": [\"0\", \"1\", \"2\", \"3\", \"7\", \"8\"], \"counts\": [1, 1, 1, 1, 1, 1], \"p_value\": 0.39023498, \"fold_change\": 5.4498405".to_string(),
        tags,
        mem: size_aligned(t_m + c_m_s + p_m, c_m_h, 8),
        sum,
        ids: None,
        p_value,
        fold_change,
    ...
    let compare_data = SummaryTest {
        ...
        print_json: "\"sum\": 6, \"samples\": [\"0\", \"1\", \"2\", \"3\", \"7\", \"8\"], \"counts\": [1, 1, 1, 1, 1, 1], \"p_value\": 0.39023498, \"fold_change\": 5.4498405".to_string(),
        tags,
        mem: size_aligned(t_m + c_m_s + em_m, c_m_h, 8),
        sum,
        ids: None,
        p_value,
        fold_change,
    ...
    let compare_data = SummaryTest {
        ...
        print_json: "\"sum\": 6, \"samples\": [\"0\", \"1\", \"2\", \"3\", \"7\", \"8\"], \"counts\": [1, 1, 1, 1, 1, 1], \"p_value\": 0.39023498, \"fold_change\": 5.4498405".to_string(),
        tags,
        mem: size_aligned(t_m + c_m_s + p_m + em_m, c_m_h, 8),
        sum,
        ids: None,
        p_value,
        fold_change,
    ...
    let compare_data = SummaryTest {
        ...
        tags,
        mem: size_aligned(t_m + c_m_s + i_m_s, c_m_h + i_m_h, 8),
        sum,
        ids:  ids.clone(),
        p_value,
        fold_change,
    ...
    let compare_data = SummaryTest {
        ...
        print_json: "\"ids\": [\"0\", \"1\", \"2\", \"3\", \"7\", \"8\"], \"sum\": 6, \"samples\": [\"0\", \"1\", \"2\", \"3\", \"7\", \"8\"], \"counts\": [1, 1, 1, 1, 1, 1], \"p_value\": 0.39023498, \"fold_change\": 5.4498405".to_string(),
        tags,
        mem: size_aligned(t_m + c_m_s + i_m_s + p_m + em_m, c_m_h + i_m_h, 8),
        sum,
        ids:  ids.clone(),
        p_value,
        fold_change,
