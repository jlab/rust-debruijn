...
use bimap::BiMap;
use debruijn::{colors::{ColorMode, Colors}, graph::DebruijnGraph, kmer::Kmer16, summarizer::TagsCountsPEMData};
...
fn test_colors() {
    ...
    graph.to_dot(
        "test_dot.dot", 
        &|node| node.node_dot_default(&colors, &config, &hashed_labels, false, ColorMode::FoldChange), 
