...
fn test_colors() {
    ...
    let node_id = 0;
    let node = graph_tcpem.get_node(node_id);
    println!("{}", node.node_dot_default(&colors, &config_tcpem, &hashed_labels_tcpem, false));
    ...
    let colors_ids = Colors::new(&graph_ids, &config_ids, ColorMode::IDS { n_ids: hashed_genes.len() });

    let node_id = 0;
    let node = graph_ids.get_node(node_id);
    println!("{}", node.node_dot_default(&colors_ids, &config_ids, &hashed_genes, false));
    ...
    graph_tcpem.to_dot(
        "test_dot.dot", 
        &|node| node.node_dot_default(&colors, &config_tcpem, &hashed_labels_tcpem, false), 
